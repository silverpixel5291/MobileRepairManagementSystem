...
export function Reports({ store }: { store: StoreType }) {
  ...
  const reports = [
    { id: 'sales', title: 'Sales Report', description: 'Invoices, gross sales, discounts, GST, payment methods, returns', icon: TrendingUp, color: 'bg-mint-500', metrics: [`₹${(totalRevenue/1000).toFixed(1)}K Revenue`, `${store.sales.length} Invoices`, `₹${(totalTax/1000).toFixed(1)}K GST`] },
    ...
    <div className="max-w-7xl mx-auto space-y-4 pb-16 lg:pb-0">
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3">
        ...
        <div className="flex items-center gap-2">
          <button className="flex items-center gap-1.5 px-3 py-2 bg-white border border-navy-200 rounded-lg text-sm text-navy-700 hover:bg-navy-50"><Calendar size={16} />Sep 2026</button>
          <button className="flex items-center gap-1.5 px-3 py-2 bg-primary-500 text-white rounded-lg text-sm font-medium hover:bg-primary-600 shadow-sm"><Download size={16} />Export CSV</button>
        </div>
      </div>
      ...
      <div className="bg-white rounded-xl border border-navy-100 shadow-sm p-5">
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
          ...
          <div className="text-center p-4 bg-navy-50 rounded-lg"><p className="text-2xl font-bold text-navy-900">{store.repairs.length}</p><p className="text-xs text-navy-500 mt-1">Repair Jobs</p></div>
