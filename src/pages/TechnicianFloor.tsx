...
export function TechnicianFloor({ store }: { store: Store }) {
  ...
  return (
    <div className="max-w-7xl mx-auto space-y-4 pb-16 lg:pb-0">
        ...
        {myJobs.map(job => {
          ...
          return (
            <div 
              ...
              className={`bg-white rounded-xl border-2 ${getStatusColor(job.status)} p-4 shadow-sm cursor-pointer hover:shadow-lg transition-all`}
            >
      ...
      {/* Work Screen - Full Screen View */}
      {selectedJobId && (() => {
        const job = store.repairs.find(r => r.id === selectedJobId);
        if (!job) return null;
        const action = getStatusAction(job.status);
